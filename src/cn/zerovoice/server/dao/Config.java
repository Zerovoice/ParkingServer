package cn.zerovoice.server.dao;

public class Config {

	public static final String HOST_ADRESS = "10.1.11.53";
	public static final int HOST_PORT = 6556;
	public static final String UPDATE_ADRESS = "http://"+HOST_ADRESS+":8080/myapp/actionupdate/";
	public static final String UPDATE_APKNAME = "Action.apk";
	public static final String UPDATE_VERJSON = "Action.txt";
	public static final String UPDATE_SAVENAME = "Action.apk";
	public static final String PACKAGENAME = "com.zerovoice";
	public static final String CACHE = "/data/data/com.zerovoice/cache";
	public static String versionJSON;
	
	private static final String TAG = "Config";
	

}
